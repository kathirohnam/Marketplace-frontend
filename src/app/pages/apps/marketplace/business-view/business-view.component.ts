...
// import { FormationDetailsDialogComponent } from "../shared/formation-details-dialog/formation-details-dialog.component";
...
export class BusinessViewComponent implements OnInit, OnDestroy {
  ...
  private getLoggedInUserId(): number {
    return Number(localStorage.getItem("login_user_id")) || 0;
  }
  ...
  getSubtitleMessage(): string {
    ...
    if (totalCompanies === 0) {
      return "Let's get your business viewed in marketplace with some initial setup.";
    } else if (pending > 0 && completed === 0) {
      return `You have ${pending} Marketplace${
        pending > 1 ? "s" : ""
      } pending. Let's complete your formations!`;
    } else if (pending > 0 && completed > 0) {
      return `Managing ${completed} active Marketplace${
        completed > 1 ? "s" : ""
      } with ${pending} more pending.`;
    } else {
      return `Successfully managing ${completed} active Marketplace${
        completed > 1 ? "s" : ""
  ...
  getDetailedStatus(stepIndex: number | undefined): string {
    if (stepIndex === undefined)
      return "Ready to begin your Marketplace formation journey";
    ...
    return (
      statusMapping[stepIndex] || "Ready to begin your Marketplace formation journey"
    );
  }
      ...
      .pipe(
        ...
        tap((event) => {
          if (event instanceof NavigationStart) {
            ...
            if (this.router.url.includes("/business-view")) {
              this.fetchProgress();
            }
            this.startPolling();
          }
